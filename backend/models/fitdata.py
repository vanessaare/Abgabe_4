...
class FITdata:
    ...
    def _pqrst_template() -> np.ndarray:
        ...
        ecg = np.zeros(FS)
        ecg += 0.15 * np.exp(-((t - 0.15) ** 2) / (2 * 0.012 ** 2))
        ecg -= 0.10 * np.exp(-((t - 0.22) ** 2) / (2 * 0.004 ** 2))
        ecg += 1.00 * np.exp(-((t - 0.25) ** 2) / (2 * 0.005 ** 2))
        ecg -= 0.15 * np.exp(-((t - 0.28) ** 2) / (2 * 0.004 ** 2))
        ecg += 0.25 * np.exp(-((t - 0.40) ** 2) / (2 * 0.025 ** 2))
        return ecg
    ...
    def _build_ecg_df(hr_df: pd.DataFrame) -> pd.DataFrame:
        ...
        template = FITdata._pqrst_template()
    ...
    def plot_time_series(self):
        return px.line(self.df, x="Zeit in ms", y="Messwerte in mV", title=f"FIT-Aktivität {self.id} – synthetisches EKG")
    ...
    def plot_with_peaks(self):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=plot_df["Zeit in ms"], y=plot_df["Messwerte in mV"], mode="lines", name="EKG (synth.)", line=dict(color="steelblue")))
        ...
        fig.update_layout(title=f"FIT-Aktivität {self.id} mit Peaks", xaxis_title="Zeit in ms", yaxis_title="mV", height=500)
        return fig
    ...
    def plot_with_peaks_window(self, start_min=0, end_min=None):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=time_sec, y=plot_df["Messwerte in mV"], mode="lines", name="EKG (synth.)", line=dict(color="steelblue")))
        peak_idx = [i for i in self.peaks if i in plot_df.index]
        if peak_idx:
            fig.add_trace(go.Scatter(
